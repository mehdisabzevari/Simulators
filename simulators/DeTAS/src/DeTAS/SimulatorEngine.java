			...
			for (int k = 0; k < nodes[i].getNumPackets(); k++) {            
                ...
                System.out.println(" target "+(k+1)+" is: "+ j);
                            if ( (nodes[i].getRank()+1) %2 == 0) {
                                 slotNumber = ran.nextInt(Queue[i]);// pick ts and ch.offset in microSchedule
                            ...
                            /*    while ( (nodes[nodes[j].getPrefferedParent()].getMicroScheduleSlotOffset() + Queue[nodes[j].getPrefferedParent()]) < slotNumber ) {
                                 ...
                                 if (slotNumber%2 != 0)
                                      slotNumber++;
                                 if (slotNumber > MAX_TIME_SLOT)
                                     slotNumber -=2;
                                 slotNumber += nodes[nodes[j].getPrefferedParent()].getMicroScheduleSlotOffset();
                                 channelOffset = ((nodes[i].getRank()+1) - 2) % 3;
                                 channelOffset += nodes[nodes[j].getPrefferedParent()].getMicroScheduleChannelOffset();
        ...
        // send num of packets as Qi to node's parent
           ...
           for (int j = 1; j < MAX_NET_SIZE; j++) {
               if (nodes[j].getPrefferedParent() !=0 && nodes[j].getPrefferedParent()!=-1 ){
                   nodes[j].setMicroScheduleChannelOffset(nodes[nodes[j].getPrefferedParent()].getMicroScheduleChannelOffset());
                   nodes[j].setMicroScheduleSlotOffset(nodes[nodes[j].getPrefferedParent()].getMicroScheduleSlotOffset());
